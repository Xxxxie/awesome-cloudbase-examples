...
基于 React、Vite 和腾讯云开发（CloudBase）的现代化 CRM 客户关系管理系统，提供客户管理、销售机会跟踪、联系人管理等完整功能。
...
- 🎨 独特的 Editorial/Brutalist 设计风格，专业且现代
- 🔄 使用 Framer Motion 实现流畅的动画效果
- 🎁 深度集成腾讯云开发 CloudBase，提供一站式后端云服务
- 📊 完整的 CRM 功能：客户管理、销售机会、联系人管理
...
访问 http://localhost:5173/ 即可直接进入 CRM 系统。
...
2. 当前环境 ID 已配置为：`tcb-advanced-a656fc`
3. 如需更换环境，请修改 `ENV_ID` 变量的值
...
├── src/
│   ├── components/       # 可复用组件
│   │   ├── Sidebar.jsx   # CRM 侧边栏导航
│   │   ├── StatCard.jsx  # 数据统计卡片
│   │   ├── Navbar.jsx    # 顶部导航栏
│   │   └── Footer.jsx    # 页脚
│   ├── pages/            # 页面组件
│   │   ├── HomePage.jsx      # 首页
│   │   ├── CRMDashboard.jsx  # CRM 数据看板
│   │   ├── CustomersPage.jsx # 客户管理页
│   │   ├── OpportunitiesPage.jsx # 销售机会页
│   │   └── ContactsPage.jsx  # 联系人管理页
│   ├── utils/            # 工具函数和云开发初始化
│   ├── App.jsx           # 应用入口
│   ├── main.jsx          # 渲染入口
│   └── index.css         # 全局样式
...
本 CRM 系统采用独特的 **Editorial/Brutalist** 设计风格：

- **大胆的排版**：使用粗体字体和紧凑的字间距
- **高对比度**：黑白为主色调，配合鲜明的强调色
- **几何图形**：简洁的图标和形状元素
- **边框设计**：粗边框和阴影效果
- **动画效果**：流畅的页面过渡和交互动画

设计理念：专业、现代、易用，避免通用 AI 美学，打造独特的视觉体验。
